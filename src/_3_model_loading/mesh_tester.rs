...
use std::rc::Rc;
use crate::{model::Model, camera::{Camera, Directions}, texture::Texture, shader::Shader};
use crate::{mesh_cube};
...
pub fn main_mesh_tester() {

    unsafe 
    {
        let event_loop = glutin::event_loop::EventLoop::new();
        let window_builder = glutin::window::WindowBuilder::new()
            .with_title("learn-opengl-glow => main_mesh_tester")
        ...
        let ourModel = Model::new(gl.clone(), "resources/objects/nanosuit/nanosuit.obj");
        
        
        let shader = Shader::new_from_files(
            gl.clone(),
            "src/_3_model_loading/shaders/mesh_tester.vs",
            "src/_3_model_loading/shaders/mesh_tester.fs"
        );

        let mesh = mesh_cube::new( gl.clone(), 0.5);
        ...
        event_loop.run(move |event, _, control_flow| {
            ...
            match event {
                Event::RedrawRequested(_) => {
                    ...
                    let model = glm::translate(&glm::Mat4::identity(), &glm::vec3(0.0, 0.2, 0.2));
