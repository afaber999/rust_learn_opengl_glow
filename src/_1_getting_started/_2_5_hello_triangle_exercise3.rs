...
pub fn main_1_2_5() {
    ...
    unsafe 
    {
        ...
        let window_builder = glutin::window::WindowBuilder::new()
            .with_title("learn-opengl-glow => _2_5_hello_triangle_exercise3")
        ...
        // ------------------------------------
        // vertex shader  1      
        let program1 = gl.create_program().expect("Cannot create program");
        ...
        gl.attach_shader(program1, vx_shader);
        ...
        gl.shader_source(fg_shader, fg_source1);
        ...
        gl.attach_shader(program1, fg_shader);

        // link shaders and check result
        gl.link_program(program1);
        if !gl.get_program_link_status(program1) {
            panic!( "{}", gl.get_program_info_log(program1));
        }        

        gl.detach_shader(program1, vx_shader);
        gl.delete_shader(vx_shader);

        gl.detach_shader(program1, fg_shader);
        ...
        // ------------------------------------
        // vertex shader  2      
        let program2 = gl.create_program().expect("Cannot create program");
        ...
        gl.attach_shader(program2, vx_shader);
        ...
        gl.shader_source(fg_shader, fg_source2);
        gl.compile_shader(fg_shader);
        ...
        gl.attach_shader(program2, fg_shader);

        // link shaders and check result
        gl.link_program(program2);
        if !gl.get_program_link_status(program2) {
            panic!( "{}", gl.get_program_info_log(program2));
        }        

        gl.detach_shader(program2, vx_shader);
        gl.delete_shader(vx_shader);

        gl.detach_shader(program2, fg_shader);
        gl.delete_shader(fg_shader);
            ...
            *control_flow = ControlFlow::WaitUntil( std::time::Instant::now() + std::time::Duration::from_millis(16));
            ...
            match event {
                Event::RedrawRequested(_) => {
                    ...
                    // select the 1st shader
                    gl.use_program(Some(program1));

                    // draw first triangle
                    gl.bind_vertex_array(Some(vao1));
                    gl.draw_arrays(glow::TRIANGLES, 0,3);

                    // select the 2nd shader
                    gl.use_program(Some(program2));
